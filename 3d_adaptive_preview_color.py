...
# Objective Function: Adaptive Hueristic Framework + Preview
...
#1 (D) Heuristic: Preview Color Use [Ignoring Spawn Location]
def preview_color_bonus_bits(state, preview_colors):
    ...
    for cells, mask in FIVE_WINDOWS:
        ...
        # cap availability so it doesn't explode
        supply_factor = min(availability[color], 8) / 8.0

        # count how many empty targets are reachable by another same-colored piece
        reachable_targets = 0
...
#1 (E) Heuristic: Completion Acesss
...
def all_possible_states_bits(state, preview_colors,
    ...
    for source_idx in iter_set_bits(state["occupied_bits"]):
        ...
        for dest_idx in iter_set_bits(reachable):
            ...
            preview_bonus = preview_color_bonus_bits(cleared_state, preview_colors)
...
def adaptive_weights(phase, features):
    ...
    if fill > 0.75:
        ...
        k *= 1.25
        beta *= 0.85
        rho *= 0.75   # preview less important when survival dominates

    elif fill < 0.25:
        beta *= 1.10
        delta_g *= 1.10
        rho *= 1.20   # preview more useful while board is still open

    # E. Preview should matter only when it is structurally relevant
    if preview_score > 20 and fill < 0.65:
        rho *= 1.20
    elif fill > 0.70:
        rho *= 0.80

    # F. Completion access / corridor unblocking situations
    ...
    # G. No obvious accessible completion -> lean more on unblocking and setup
    if no_obvious_clear:
        eta *= 1.20
        delta_g *= 1.10
...
def greedy(current_node, preview_colors):
    features = extract_state_features(current_node, preview_colors)
    phase = game_phase(features)

    alpha, beta, gamma, rho, eta, delta_g, delta_p, k = adaptive_weights(phase, features)
