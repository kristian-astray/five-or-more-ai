...
# Objective Function: Composite Heuristic
...
def evaluate_weights_bitboard(alpha, beta, gamma, delta, epsilon, eta, k, num_runs=10):
    scores = []
...
# Parameters were obtained from personal experimenation before final grid search
def grid_search_bitboard():
    alpha_values = [0.4, 0.6]
    beta_values = [0.05, 0.1]
    gamma_values = [0.003, 0.005]
    delta_values = [0.04, 0.06]
    epsilon_values = [0.0005, 0.001]
    eta_values = [0.08]
    k_values = [9]
                    ...
                    for epsilon in epsilon_values:
                        for eta in eta_values:
                            for k in k_values:
                                ...
                                avg_score, best_score, scores = evaluate_weights_bitboard(
                                    alpha, beta, gamma, delta, epsilon, eta, k, num_runs=10
                                )
                                ...
                                print(
                                    ...
                                    f"delta={delta}, epsilon={epsilon}, eta={eta}, k={k} | "
                                    f"Avg={avg_score:.2f}, Best={best_score}"
                                )

                                if avg_score > best_avg:
                                    best_avg = avg_score
                                    best_params = (alpha, beta, gamma, delta, epsilon, eta, k)

    print("\nBest parameters:")
    print(f"alpha={best_params[0]}")
    print(f"beta={best_params[1]}")
    print(f"gamma={best_params[2]}")
    print(f"delta={best_params[3]}")
    print(f"epsilon={best_params[4]}")
    print(f"eta={best_params[5]}")
    print(f"k={best_params[6]}")
