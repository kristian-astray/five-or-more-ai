# Heuristic: Delta Path
# Better considers unblocking moves that allow for increase general mobility of pieces on the board
...
#1 (e) Adjacent Path Based Mobility via BFS [Only counts mobility based on useful moves that allow adjacent square placement]
    ...
    for sr, sc in occupied:
        for dr, dc in empty:
            if check_move(current_node, (sr, sc), (dr, dc)):
                ...
                delta_path = path_mobility - path_mobility_score(current_node)

                cleared_bonus = 5 * sum(POINT_SYSTEM.get(L, 0) for L in cleared)
                heuristic_score = 1.0 * line_potential + 0.1 * gap_potential + 0.001 * delta_path + cleared_bonus  
                1.0, 0.1, 0.005
