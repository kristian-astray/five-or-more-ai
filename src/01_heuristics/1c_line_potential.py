# Heuristic: Line Potential + Cleared Bonus
# [Progressively builds lines of increasing len towards lines of length 5 or More]
    ...
    for sr, sc in occupied:
        for dr, dc in empty:
            if check_move(current_node, (sr, sc), (dr, dc)):
                ...
                #heuristic_score = h_score(temp_node)                                        ##IMPORTANT: VERSION 1 calculate pre-clearing line potential
                new_node, cleared = five_more_clear(temp_node, (dr,dc))

                cleared_bonus = 5 * sum(POINT_SYSTEM.get(L, 0) for L in cleared)
                heuristic_score = h_score(new_node) + cleared_bonus                          ##IMPORTANT: VERSION 2 take into account post-clearing (need to add cleared bonus to promote clearing)   
