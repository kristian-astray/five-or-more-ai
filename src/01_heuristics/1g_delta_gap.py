# Heuristic: Delta Gap
# Enables unblocking cause moves that for example move B in RRRBR will result in RRR-R which will show as an increase in gap potential
    ...
    for sr, sc in occupied:
        for dr, dc in empty:
            if check_move(current_node, (sr, sc), (dr, dc)):
                ...
                delta_gap = gap_potential - gap_potential_score(current_node) 

                cleared_bonus = 9 * sum(POINT_SYSTEM.get(L, 0) for L in cleared)
                heuristic_score = line_potential + 0.01 * gap_potential + 0.1 * delta_gap + cleared_bonus                           
