# Heuristic: Delta Access
# Change in ability to complete near-finished lines after a move, rewards moves that unblock and allow for actionable configurations/sequences
...
## --- Not sure what to do with values 5+. Currently I just want it to greedily try make lines of 5 ---
...
#1 (e) Heuristic: Completion Access
    ...
    for sr, sc in occupied:
        for dr, dc in empty:
            if check_move(current_node, (sr, sc), (dr, dc)):
                ...
                delta_access = completion_access_score(new_node) - completion_access_score(current_node)

                cleared_bonus = 9 * sum(POINT_SYSTEM.get(L, 0) for L in cleared)
                heuristic_score = line_potential + 0.1 * gap_potential + 0.05 * delta_access + cleared_bonus                           
