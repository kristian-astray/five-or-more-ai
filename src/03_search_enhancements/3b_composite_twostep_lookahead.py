...
# Objective Function: Composite Heuristic + 2 Step-lookahead (ignoring spawn)
...
#1 (E) Heuristic: Two-Step Lookahead [Ignoring Spawn]
# Note. in the final model we consider the heuristic score of the first move + a weighted heuristic score of the second move. This is to try mitigate over optimism and blindly chase future 2nd step move strategy
def two_step_composite_bonus_bits(
    state,
    alpha=0.4, beta=0.1, gamma=0.005,
    delta=0.06, epsilon=0.001, eta=0.08, k=9,
    max_second_moves=120
):
    """
    Evaluates the best second move from the post-first-move state using
    the same composite heuristic. Spawn is ignored between the two moves.
    """
...
def all_possible_states_bits(
    state, alpha, beta, gamma, delta, epsilon, eta, k,
    lookahead_weight=0.15, top_k=10
):
    possible_states = []
    ...
    for board_state, move, base_score, cleared in top_states:
        lookahead_bonus = two_step_composite_bonus_bits(
            board_state,
...
#3 Greedy Algorithm
def greedy(current_node, next_colors=None, alpha=0.4, beta=0.1, gamma=0.005,
           delta=0.06, epsilon=0.001, eta=0.08, k=9,
           lookahead_weight=0.15, top_k=10):
