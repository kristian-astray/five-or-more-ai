...
# Objective Function: Adaptive Heuristic Framework + Monte Carlo Simulation
...
def all_possible_states_bits(state,
...
#3 (d) Adaptive one-step greedy score after simulated spawn
def best_one_step_adaptive_score_bits(state):
    features = extract_state_features(state)
    phase = game_phase(features)

    alpha, beta, gamma, eta, delta_g, delta_p, k = adaptive_weights(
        phase,
        features
    )
...
#3 (e) Monte Carlo spawn simulation + one-step adaptive greedy evaluation
def monte_carlo_spawn_then_adaptive_greedy_bits(
    state,
    next_colors,
    num_rollouts=10
):
    ...
    for _ in range(num_rollouts):
        ...
        # clear lines formed by spawned tiles
        ...
        # adaptive one-step greedy evaluation after spawn
        future_score = best_one_step_adaptive_score_bits(sim_state)
...
#3 (f) Greedy Algorithm + Monte Carlo when no top candidate clears
def greedy(current_node, next_colors, top_k=10, num_rollouts=10,
           monte_carlo_weight=1.0, mc_fill_threshold=0.5):
    ...
    # If any top candidate clears, skip Monte Carlo entirely
    any_clear = any(len(cleared) > 0 for _, _, _, cleared in candidate_states)
    high_stress = features["fill_ratio"] >= mc_fill_threshold
    ...
    for board_state, move, heuristic_score, cleared in candidate_states:
        ...
        if use_monte_carlo:
            mc_score = monte_carlo_spawn_then_adaptive_greedy_bits(
                board_state,
                next_colors,
...
def run_single_game(print_game=False):
    ...
    while game_board["occupied_bits"].bit_count() < NUM_CELLS:
        ...
        best_board, best_move, _, len_cleared = greedy(game_board, next_colors)
