...
# Objective Function: Composite Heuristic + Preview Colour
...
def all_possible_states_bits(state, preview_colors, alpha, beta, gamma, rho, delta, epsilon, eta, k):
    possible_states = []
...
def greedy(current_node, next_colors, alpha=0.4, beta=0.1, gamma=0.005,
           rho=0.04, delta=0.06, epsilon=0.001, eta=0.08, k=9):

    states = all_possible_states_bits(
        current_node,
        next_colors,
        alpha, beta, gamma, rho, delta, epsilon, eta, k
    )
