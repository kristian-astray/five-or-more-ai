...
# Objective Function: Adaptive Hueristic Framework + 2 Step-Lookahead Ignoring Spawn
...
def completion_access_score_bits(state):
    ...
    for cells, mask in FIVE_WINDOWS:
        ...
        # map each target square to whether some outside same-colored piece can reach it
        reachable_targets = 0
        ...
        for target_idx in empty_targets:
            ...
            for source_idx in iter_set_bits(state["color_bits"][color]):
                if source_idx in cells:
                    continue  # skip pieces already inside the window
        ...
        if n == 4:
            if reachable_targets == 1:
                total += 40   # real actionable 4-line
            else:
                total += 2    # fake / inaccessible 4-line

        elif n == 3:
            if reachable_targets == 2:
                total += 14   # strong accessible setup
            elif reachable_targets == 1:
                total += 6    # partial accessibility
            else:
                total += 1    # structural only, not actionable
...
def all_possible_states_bits(
    ...
    for source_idx in iter_set_bits(state["occupied_bits"]):
        ...
        for dest_idx in iter_set_bits(reachable):
            ...
            possible_states.append((cleared_state, move, base_score, cleared))

    # Top-k pruning before expensive second-step evaluation
    possible_states.sort(key=lambda x: x[2], reverse=True)
    top_states = possible_states[:top_k]

    reranked_states = []
    for board_state, move, base_score, cleared in top_states:
        lookahead_bonus = two_step_adaptive_bonus_bits(board_state)
        final_score = base_score + lookahead_weight * lookahead_bonus
...
#3 (b) Game Phase Tracker [Early, Middle, Late]
...
#3 (c) Adaptive/ State-Dependent Weights Function
def adaptive_weights(phase, features):
    ...
    no_obvious_clear = access_score == 0  # rough indicator: no reachable 4-line right now
    ...
    # --- 2. State-dependent adjustments ---

    # A. Mobility crisis / path restriction
    ...
    # B. Strong latent gap opportunities
    if gap_score > 180:
        beta *= 1.20
        delta_g *= 1.25
    ...
    # C. Strong existing line structure
    if line_score > 45:
        alpha *= 1.10
        k *= 1.05
    ...
    # E. Completion access / corridor unblocking situations
    ...
    # F. No obvious accessible completion -> lean more on unblocking and setup
    if no_obvious_clear:
        eta *= 1.20
        delta_g *= 1.10

    return alpha, beta, gamma, eta, delta_g, delta_p, k


#3 (d) Two-Step Lookahead [Ignoring Spawn]
def two_step_adaptive_bonus_bits(state, max_second_moves=120):

    features = extract_state_features(state)
    phase = game_phase(features)

    alpha, beta, gamma, eta, delta_g, delta_p, k = adaptive_weights(phase, features)
    ...
    for source_idx in iter_set_bits(state["occupied_bits"]):
        ...
        for dest_idx in iter_set_bits(reachable):
            ...
            line_potential = line_potential_score_bits(next_state)
            gap_potential = gap_potential_score_bits(next_state)
            path_mobility = path_mobility_score_bits(next_state)
            access_score = completion_access_score_bits(next_state)
            ...
            examined += 1
            if examined >= max_second_moves:
                return best_score

    return best_score if best_score != -float("inf") else 0


#3 (d) Greedy Algorithm
def greedy(current_node, lookahead_weight=0.15, top_k=10):
    features = extract_state_features(current_node)
    phase = game_phase(features)
    ...
    best_board, best_move, _, cleared = random.choice(tied_states)
